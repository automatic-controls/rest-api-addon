...
public class SerializationStream {
   ...
   * <ul><li>Required capacity: {@code arr.length+4}</li></ul>
   */
  public void write(byte[] arr, boolean obfuscate, boolean clone){
    if (obfuscate){
      byte[] temp = clone?arr.clone():arr;
  ...
  public void write(String str, boolean obfuscate){
    write(str.getBytes(StandardCharsets.UTF_8), obfuscate, false);
  }
  /**
   ...
   */
  public byte[] readBytes(boolean obfuscate){
    byte[] arr = readBytes();
    if (obfuscate){
  ...
  public String readString(boolean obfuscate){
    return new String(readBytes(obfuscate), StandardCharsets.UTF_8);
  }
  /**
